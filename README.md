...
在 Web 管理界面点击「查看订阅链接」，会提供可直接用于 **Surge** 的转换地址，格式为：

```
https://sublink.eooce.com/surge?config=你的基础订阅链接
```

也可带优选参数：

```
https://sublink.eooce.com/surge?config=你的基础订阅链接?CFIP=优选IP&CFPORT=端口
```

在 Surge 中选择「配置」→「从 URL 下载」粘贴上述链接即可。转换服务默认使用 `https://sublink.eooce.com`，可通过环境变量 `API_URL` 更换。
