...
// 可选：外部访问地址，用于保活探活（如 https://your-domain.com）
...
function decodeBase64Content(base64Content) {
    ...
    // 尝试 base64 解码；失败则返回原文（部分订阅直接返回明文节点）
    try {
        const decoded = Buffer.from(trimmed, 'base64').toString('utf-8');
        // 简单校验：解码后应包含常见协议头或看起来像节点列表
        if (decoded.includes('://') || decoded.includes('vmess') || decoded.includes('vless') || decoded.includes('trojan') || decoded.includes('ss://') || decoded.includes('hysteria')) {
            return decoded;
        }
        // 可能本身就是明文
